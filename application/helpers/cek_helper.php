...
{
    $CI        = &get_instance();
    $userId    = $CI->session->userdata('id_konsumen');

    if ($userId) {
        $query    = $CI->db->where('id_konsumen', $userId)->count_all_results('cart');
        return $query;
    }
...
function title($page = null)
{
    $ci = &get_instance();
    if ($page == null) {
        $title = $ci->db->query("SELECT nama_website FROM admin_website")->row_array();
        return $title['nama_website'];
    } else {
        $title = $ci->db->query("SELECT nama_website FROM admin_website")->row_array();
        return $page . " - " . $title['nama_website'];
    }
}
